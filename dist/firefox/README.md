...
## Use As a New Tab Extension

This repo includes two manifests:
- `manifest.json` for Chrome/Edge (and Firefox temporary loading)
- `manifest.opera.json` for Opera (Opera does not allow `chrome_url_overrides.newtab` for normal extensions)
...
This generates:
- `dist/chrome` (uses `manifest.json`)
- `dist/firefox` (uses `manifest.json`)
- `dist/opera` (uses `manifest.opera.json` renamed to `manifest.json`)

Chrome uses a `focus.html -> index.html` redirect for reliable initial input focus on New Tab.

### Chrome / Edge / Opera (Chromium)
1. Go to `chrome://extensions` (Edge: `edge://extensions`, Opera: `opera://extensions`).
2. Enable **Developer mode**.
3. Chrome/Edge: click **Load unpacked** and select `dist/chrome`.
4. Opera: click **Load unpacked** and select `dist/opera`.
4. Open a new tab (`Ctrl+T`) to use the start page.

### Opera behavior note
- Opera blocks direct New Tab override via `chrome_url_overrides`.
- The Opera manifest uses a background fallback:
  - redirects newly created start-page tabs to your extension page,
  - adds toolbar button support,
  - adds shortcut `Ctrl+Shift+Y` to open your start page directly.
